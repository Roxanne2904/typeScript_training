...
// Créez une interface "Person" avec les propriétés "name" de type chaîne de caractères et "age" de type nombre.
interface Person {
	name: string;
	age: number;
}
...
function add01<T extends number, U extends number>(arg1: T, arg2: U): number {
	return arg1 + arg2;
}

console.log("add function", add(23, 45));
console.log("add01 function", add01<67, 45>(67, 45));

// Créez une classe "Animal" avec une propriété "name" de type chaîne de caractères et une méthode "makeSound" qui renvoie une chaîne de caractères.

type Animals = "horse" | "pig" | "firefly" | "cat" | "dog";
...
const simulatedAsyncOperation = (): Promise<string> =>
	new Promise((resolve: (arg: string) => void) => {
		setTimeout(
			() => resolve("Opération asynchrone réussie après 2 secondes"),
			2000
		);
	});

console.log(
	"simulatedAsyncOperation",
	simulatedAsyncOperation().then((result) => console.log(result))
);
